...
var (
	...
	flagBlockDeviceFSUUID = flag.String("device", "", "FS_UUID of the device where the kernel (and optionally initramfs) are located. Ignored if -grub is set or if -kernel is not specified")
)
...
func main() {
	...
	if *flagBlockDeviceFSUUID != "" && *flagDeviceGUID != "" {
		log.Fatal("Options -guid and -device are mutually exclusive")
	}
