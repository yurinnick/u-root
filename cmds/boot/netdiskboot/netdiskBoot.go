...
func main() {
	...
	if !*dryRun {
		...
		err = DownloadFile("/tmp/"+imageName+".xz", cfg.ImgURL)
		if err != nil {
			fmt.Printf("Downloading file at %s failed with %v\n", cfg.ImgURL, err)
		}

		data, err := ioutil.ReadFile("/tmp/" + imageName + ".xz")
		if err != nil {
			log.Fatal(err)
			return
		}
		...
		cmd := exec.Command("dd if=/tmp/" + imageName + "of=" + cfg.Device + " bs=4096")
